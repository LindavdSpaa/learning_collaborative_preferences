...
class GraspController:
  def gripper_pos_callback(self, data):
    ...
    self.curr_state = 0 if pos > 0.01/2 else 1 if pos > 0.006/2 else -1
    self.state_pub.publish(self.curr_state)

  def open(self, dOpen=0.01): # open distance [m]
  ...
  def __init__(self) -> None:
    ...
    self.grip_command.goal.epsilon.inner = 0.
    self.grip_command.goal.epsilon.outer = 0.
    self.grip_command.goal.force = 0.
    self.grip_command.goal.width = 0.1
  ...
  def run(self) -> None:
    r = rospy.Rate(20)
    while not rospy.is_shutdown():
      if self.goal_state == 0 and self.curr_state != 0 and self.last_grip_command != False:
        self.open()
        ...
        if self.curr_state < 0 and self.last_grip_command:
          ...
          self.open()
        elif self.curr_state == 0 and self.last_grip_command != True:
          self.close()
